...
def get_merged_model_dynamic_axes(input_names: list[str], output_names: list[str]):
    ...
    for name in input_names + output_names:
        if name in {"input_ids", "position_ids"}:
            ...
            # shape is (batch_size, sequence_length, vocab_size)
            dynamic_axes[name] = {0: "batch_size", 1: "sequence_length"}
        elif name == "medusa_logits":
            # shape is (batch_size, sequence_length, vocab_size)
            dynamic_axes[name] = {0: "num_medusa", 1: "batch_size", 2: "sequence_length"}
        elif "present" in name:
        ...
        elif "medusa_mask" in name:
            dynamic_axes[name] = {2: "sequence_length", 3: "sequence_length"}
        else:
            raise Exception("Unknown input or output name found")
    return dynamic_axes
...
def run_dynamo_export(
    ...
    batch_size, sequence_length, past_sequence_length = 2, 100, 0
